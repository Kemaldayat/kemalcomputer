        ...
        window.togglePembayaranFields = () => {
            const val = document.getElementById('setTipePembayaran').value;
            const group = document.getElementById('manualPaymentGroup');
            if (group) {
                group.style.display = val === 'manual' ? 'block' : 'none';
            }
        };

        window.uploadQrisImage = async (inputElem) => {
            if (inputElem.files.length === 0) return;
        ...
        async function loadWebsiteSettings() {
            try {
                ...
                if (snapshot.exists()) {
                    ...
                    if (document.getElementById('setTipePembayaran')) document.getElementById('setTipePembayaran').value = data.tipePembayaran || 'manual';
                    ...
                    if (document.getElementById('previewQris')) {
                        if (data.qrisUrl) {
                            ...
                            document.getElementById('previewQris').style.display = 'none';
                        }
                    }
                    if (window.togglePembayaranFields) window.togglePembayaranFields();
            ...
            onValue(ref(db, 'antrian'), (snapshot) => {
                ...
                if (snapshot.exists()) {
                    snapshot.forEach((child) => {
                        ...
                        let payStatus = s.payment_status || 'Belum Dibayar';
