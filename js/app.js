...
window.trackService = async (kodeParam = null) => {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            // === STEP TRACKER LOGIC ===
            // Warna konsisten: completed = teal, active = sesuai step, pending = abu
            const DONE_COLOR   = '#0d9488'; // teal primer
            const ACTIVE_COLORS = { Menunggu: '#f59e0b', Proses: '#3b82f6', Selesai: '#10b981', Diambil: '#8b5cf6' };
            const steps = [
                { key: 'Menunggu', label: 'Antrian', icon: 'fas fa-hourglass-half',  activeIcon: 'fas fa-cog', desc: 'Dalam antrian' },
                { key: 'Proses',   label: 'Proses',  icon: 'fas fa-microchip',        activeIcon: 'fas fa-cog', desc: 'Dikerjakan teknisi' },
                { key: 'Selesai',  label: 'Selesai', icon: 'fas fa-check-double',     activeIcon: 'fas fa-cog', desc: 'Perbaikan selesai' },
                { key: 'Diambil',  label: 'Diambil', icon: 'fas fa-box-open',         activeIcon: 'fas fa-cog', desc: 'Sudah diambil' },
            ];
            const statusOrder = ['Menunggu', 'Proses', 'Selesai', 'Diambil'];
            const currentStepIdx = statusOrder.indexOf(service.status);
            ...
            const stepperHTML = steps.map((step, i) => {
                ...
                const activeColor = ACTIVE_COLORS[step.key] || DONE_COLOR;
                let circleStyle = '';
                let labelStyle  = '';
                let pulseHtml   = '';
                let checkHtml   = '';
                ...
                if (isDone) {
                    // Step selesai: lingkaran teal + centang
                    circleStyle = `background: ${DONE_COLOR}; border-color: ${DONE_COLOR}; box-shadow: 0 0 0 4px ${DONE_COLOR}22;`;
                    checkHtml   = `<i class="fas fa-check" style="color:white; font-size:1.05rem;"></i>`;
                    labelStyle  = `color:${DONE_COLOR}; font-weight:700;`;
                } else if (isActive) {
                    // Step aktif: warna step, icon fa-cog berputar, pulse ring diaktifkan
                    circleStyle = `background: ${activeColor}; border-color: ${activeColor};`;
                    checkHtml   = `<i class="fas fa-cog kc-spin-anim" style="color:white; font-size:1.1rem;"></i>`;
                    labelStyle  = `color:${activeColor}; font-weight:700;`;
                    pulseHtml   = `<span class="kc-active-dot" style="background:${activeColor};"></span>`;
                    activePulseClass = `kc-active-pulse-${step.key.toLowerCase()}`;
                } else {
                    // Step belum dicapai: abu transparan
                    circleStyle = `background: rgba(255,255,255,0.06); border-color: rgba(255,255,255,0.12);`;
                    checkHtml   = `<i class="${step.icon}" style="color:rgba(255,255,255,0.22); font-size:1rem;"></i>`;
                    labelStyle  = `color: rgba(255,255,255,0.28); font-weight:500;`;
                }

                // Konektor antar step — selalu pakai warna teal agar konsisten
                const connFillColor = isDone ? DONE_COLOR : (isActive ? activeColor : 'rgba(255,255,255,0.1)');
                const targetWidth = isDone ? 100 : (isActive ? 55 : 0);
                const connectorHtml = i < steps.length - 1 ? `
                    <div class="kc-step-connector">
                        <div class="kc-step-connector-fill" style="--target-width: ${targetWidth}%; background:${connFillColor}; animation-delay: ${i * 0.2 + 0.3}s;"></div>
                    </div>` : '';

                return `
                    <div class="kc-step-item">
            ...
            // Trophy / confetti if done or picked up
            const celebrateHtml = (activeIdx >= 2) ? `
                <div class="kc-celebrate" id="kc-celebrate-banner">
                    ${activeIdx === 3
                        ? `<span style="font-size:1.5rem;">📦</span> <span>Perangkat Anda sudah diambil. Terima kasih telah mempercayai kami!</span>`
                        : `<span style="font-size:1.5rem;">🎉</span> <span>Perbaikan selesai! Silakan datang mengambil perangkat Anda.</span>`
                    }
                </div>` : '';
            ...
            trackingContent.innerHTML = `
                <style>
                    ...
                    .kc-step-circle {
                        ...
                        position: relative;
                        cursor: default;
                        animation: kc-pop-in 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) both;
                    }
                    .kc-step-connector {
                        flex: 1;
                        height: 3px;
                    ...
                    .kc-step-connector-fill {
                        ...
                        border-radius: 999px;
                        width: 0%;
                        animation: kc-line-grow 1.2s cubic-bezier(.4,0,.2,1) both;
                    }
                    .kc-step-label {
                        font-size: 0.72rem;
                        font-weight: 600;
                    ...
                    /* ── Pulse animations per active step ── */
                    .kc-active-pulse-menunggu {
                        animation: kc-pop-in 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) both, kc-pulse-menunggu 2s infinite 0.6s;
                    }
                    .kc-active-pulse-proses {
                        animation: kc-pop-in 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) both, kc-pulse-proses 2s infinite 0.6s;
                    }
                    .kc-active-pulse-selesai {
                        animation: kc-pop-in 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) both, kc-pulse-selesai 2s infinite 0.6s;
                    }
                    .kc-active-pulse-diambil {
                        animation: kc-pop-in 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) both, kc-pulse-diambil 2s infinite 0.6s;
                    }

                    @keyframes kc-pulse-menunggu {
                        0%, 100% { box-shadow: 0 0 0 0px rgba(245, 158, 11, 0.4); }
                        50%      { box-shadow: 0 0 0 10px rgba(245, 158, 11, 0); }
                    }
                    @keyframes kc-pulse-proses {
                        0%, 100% { box-shadow: 0 0 0 0px rgba(59, 130, 246, 0.4); }
                        50%      { box-shadow: 0 0 0 10px rgba(59, 130, 246, 0); }
                    }
                    @keyframes kc-pulse-selesai {
                        0%, 100% { box-shadow: 0 0 0 0px rgba(16, 185, 129, 0.4); }
                        50%      { box-shadow: 0 0 0 10px rgba(16, 185, 129, 0); }
                    }
                    @keyframes kc-pulse-diambil {
                    ...
                    @keyframes kc-line-grow {
                        from { width: 0%; }
                        to   { width: var(--target-width, 0%); }
                    }
                    @keyframes kc-fade-in-label {
                    ...
                    .kc-spin-anim { animation: kc-spin 2s linear infinite; display: inline-block; }
                    .kc-celebrate {
                        display: flex;
                        align-items: center;
                        gap: 0.75rem;
                        ...
                        font-weight: 500;
                        color: #d1fae5;
                        animation: kc-fade-slide-in 0.7s cubic-bezier(.34,1.56,.64,1) both;
                    }
                    @keyframes kc-fade-slide-in {
